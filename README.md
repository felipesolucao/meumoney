...
```bash
npx prisma migrate dev --name modulo_login
```

Como isso adiciona colunas obrigatórias em tabelas que já têm dados de
teste, o Prisma **não consegue aplicar isso sem perder dados** — ele vai
avisar disso no terminal e perguntar se pode resetar o banco de
desenvolvimento. Digite `y` (sim) pra confirmar. Isso apaga todo o
conteúdo do banco, cria a migração, aplica ela, e deixa o banco pronto pra
receber o `npm run db:seed` de novo (que cria a conta de admin
